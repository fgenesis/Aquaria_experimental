...
PEXCEPTION_POINTERS pExceptionInfo )
{
    ...
    sprintf(m_szLogFileName, "%s\\%s_[%u-%u_%u-%u-%u].txt",
        module_folder_name, pos, systime.wDay, systime.wMonth, systime.wHour, systime.wMinute, systime.wSecond
        );

    TCHAR msg[MAX_PATH + 512];
    sprintf(msg, "Oops, the application crashed. A crashdump will be written to '%s', please send it to:\n   fg@fg.kicks-ass.org", m_szLogFileName);

    MessageBox(0, msg, "OH GODS IT EXPLODED", MB_OK | MB_ICONERROR);
